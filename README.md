...
| **comments** | Artalk comments (widget + API + moderation) | `/` (API), `/admin/` (dashboard) | `https://comments-production-xxxx.up.railway.app` |
...
- **Artalk comments** — Self-hosted comment system with SQLite storage on a persistent volume (PostgreSQL/MySQL optional via `ARTALK_DB_TYPE` + `DATABASE_URL`). Moderation dashboard at `/admin/` on the comments service URL.
...
3. After deploy, note the three service URLs from Railway's dashboard:
   ...
   - **Comments**: comments service domain → `/admin/` for moderation
...
Open the **comments** service URL → `/admin/` → log in with `ARTALK_ADMIN_EMAIL` / `ARTALK_ADMIN_PASSWORD`
